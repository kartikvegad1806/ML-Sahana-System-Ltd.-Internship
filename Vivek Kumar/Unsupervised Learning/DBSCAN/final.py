...
RANDOM_STATE       = 42
DATASET_PATH       = "Clustering_gmm (2).csv"
...
# Output
OUTPUT_CSV         = "dbscan_clustered.csv"
...
class DatasetLoader:
    """
    Loads the clustering dataset from CSV or generates a synthetic fallback.

    The synthetic dataset contains 3 Gaussian clusters (Weight vs Height)
    plus a small scatter of noise points, mimicking a real-world scenario.
    """
    ...
    def _generate_synthetic_dataset(self, n_samples: int = 400) -> pd.DataFrame:
        np.random.seed(RANDOM_STATE)

        # Cluster 0: Light / Short
        n0 = n_samples // 3
        w0 = np.random.normal(55, 5,  n0)
        h0 = np.random.normal(160, 5, n0)

        # Cluster 1: Medium / Medium
        n1 = n_samples // 3
        w1 = np.random.normal(75, 6,  n1)
        h1 = np.random.normal(175, 5, n1)

        # Cluster 2: Heavy / Tall
        n2 = n_samples - n0 - n1
        w2 = np.random.normal(95, 6,  n2)
        h2 = np.random.normal(185, 5, n2)

        # Noise scatter
        n_noise = 20
        wn = np.random.uniform(40, 120, n_noise)
        hn = np.random.uniform(145, 200, n_noise)

        weights = np.concatenate([w0, w1, w2, wn])
        heights = np.concatenate([h0, h1, h2, hn])

        df = pd.DataFrame({'Weight': np.round(weights, 1),
        ...
        if self.file_path and os.path.exists(self.file_path):
            try:
                ...
                if required.issubset(set(df.columns)):
                    self.data = df
                    print(f"[OK] Loaded from  : {self.file_path}")
                ...
                print(f"[WARN] Failed to load {self.file_path}: {ex}")
...
class ClusteringVisualizer:
    ...
    def _plot_missing_values(self):
        ...
        plt.tight_layout()
        plt.savefig("dbscan_missing_values.png", dpi=DPI, bbox_inches='tight')
        plt.close()
    ...
    def _plot_raw_scatter(self):
        ...
        plt.tight_layout()
        plt.savefig("dbscan_raw_scatter.png", dpi=DPI, bbox_inches='tight')
        plt.close()
    ...
    def _plot_feature_distributions(self):
        ...
        plt.tight_layout()
        plt.savefig("dbscan_feature_distributions.png", dpi=DPI, bbox_inches='tight')
        plt.close()
    ...
    def _plot_boxplots(self):
        ...
        plt.tight_layout()
        plt.savefig("dbscan_boxplots.png", dpi=DPI, bbox_inches='tight')
        plt.close()
    ...
    def _plot_correlation_heatmap(self):
        ...
        plt.tight_layout()
        plt.savefig("dbscan_correlation_heatmap.png", dpi=DPI, bbox_inches='tight')
        plt.close()
...
class DBSCANHyperparameterTuner:
    ...
    def _plot_tuning_heatmaps(self):
        ...
        plt.tight_layout()
        plt.savefig("dbscan_tuning_heatmaps.png", dpi=DPI, bbox_inches='tight')
        plt.close()
...
class DBSCANModel:
    ...
    def plot_k_distance(X_scaled: np.ndarray, k: int = 4):
        ...
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig("dbscan_k_distance.png", dpi=DPI, bbox_inches='tight')
        plt.close()
...
class ModelEvaluator:
    ...
    def _plot_cluster_result(self, X_raw: np.ndarray):
        ...
        plt.tight_layout()
        plt.savefig("dbscan_cluster_result.png", dpi=DPI, bbox_inches='tight')
        plt.close()
    ...
    def _plot_noise_vs_clusters(self, X_raw: np.ndarray):
        ...
        plt.tight_layout()
        plt.savefig("dbscan_noise_vs_clusters.png", dpi=DPI, bbox_inches='tight')
        plt.close()
    ...
    def _plot_cluster_distributions(self, df: pd.DataFrame):
        ...
        plt.tight_layout()
        plt.savefig("dbscan_cluster_distributions.png", dpi=DPI, bbox_inches='tight')
        plt.close()
    ...
    def _plot_pca_projection(self, X_scaled: np.ndarray):
        ...
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig("dbscan_pca_projection.png", dpi=DPI, bbox_inches='tight')
        plt.close()
    ...
    def _plot_metrics_summary(self, metrics: DBSCANMetrics):
        ...
        plt.tight_layout()
        plt.savefig("dbscan_metrics_summary.png", dpi=DPI, bbox_inches='tight')
        plt.close()
...
class MLPipeline:
    ...
    def run(self):
        print(f"\n{'=' * 70}")
        print("DBSCAN PIPELINE --- DENSITY-BASED CLUSTERING")
        print(f"{'=' * 70}")
    ...
    def _predict_new_point(self):
        ...
        new_point = {FEATURES[0]: 72.0, FEATURES[1]: 175.0}
